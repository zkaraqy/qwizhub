    ...
    <div v-else>
      <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2">
          ...
          <button class="btn btn-success fw-semibold" @click="showPublishModal = true" :disabled="questionnaire.status !== 'draft' || questions.length === 0">
            Publikasikan
          </button>
        </div>
      </div>
        ...
        <div class="card-body p-4">
          <div class="row">
            <div class="col-md-6 mb-3">
              ...
              <input type="text" class="form-control" v-model="questionnaire.topic" :disabled="questionnaire.status !== 'draft'">
            </div>
            <div class="col-md-6 mb-3">
              <label class="form-label fw-semibold">Tujuan Penelitian</label>
              <input type="text" class="form-control" v-model="questionnaire.researchObjective" :disabled="questionnaire.status !== 'draft'">
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="d-flex justify-content-between align-items-center mb-3 mt-5">
        ...
        <div class="d-flex gap-2">
          <button class="btn btn-outline-primary" @click="generateByAI" :disabled="generatingAI || questionnaire.status !== 'draft'">
            ...
            <div class="card-body p-4">
              <div class="d-flex justify-content-between align-items-start">
                <div class="flex-grow-1">
                  ...
                  <div class="mb-2">
                    <span class="badge bg-primary bg-opacity-10 text-primary me-2">{{ getQuestionTypeLabel(question.questionType) }}</span>
                    <span v-if="question.source === 'ai_generated'" class="badge bg-info bg-opacity-10 text-info">AI Generated</span>
                  </div>
                  
                  <div v-if="['multiple_choice', 'checkbox', 'dropdown', 'closed', 'filter'].includes(question.questionType) && question.options && question.options.length > 0" class="mt-3">
                ...
                <div class="ms-3" v-if="questionnaire.status === 'draft'">
        ...
        <div class="modal-content rounded-4 border-0 shadow">
          ...
          <div class="modal-body p-4">
            ...
            <div class="mb-3">
              ...
              <textarea v-model="questionForm.questionText" class="form-control rounded-3" rows="3" placeholder="Tuliskan pertanyaan..."></textarea>
            </div>
            
            <div v-if="['multiple_choice', 'checkbox', 'dropdown', 'closed', 'filter', 'mixed'].includes(questionForm.questionType)" class="mb-3">
              <label class="form-label fw-semibold">Opsi Jawaban (pisahkan dengan koma)</label>
              <input type="text" class="form-control rounded-3" v-model="questionForm.optionsInput" placeholder="Sangat Baik, Baik, Cukup, Kurang">
            </div>
          </div>
        ...
        <div class="modal-content rounded-4 border-0 shadow">
          ...
          <div class="modal-body p-4">
            ...
            <div class="mb-4">
              ...
              <input type="number" class="form-control rounded-3" v-model.number="publishForm.honorariumPerRespondent" min="0" step="1000">
            </div>
            
            <div class="bg-light p-3 rounded-4 mb-3">
              <div class="d-flex justify-content-between mb-2">
                <span class="text-muted">Total Honor ({{ publishForm.targetRespondents || 0 }} x Rp {{ publishForm.honorariumPerRespondent || 0 }})</span>
                <span class="fw-semibold">Rp {{ ((publishForm.targetRespondents || 0) * (publishForm.honorariumPerRespondent || 0)).toLocaleString('id-ID') }}</span>
              </div>
              <div class="d-flex justify-content-between mb-2">
              ...
              <div class="d-flex justify-content-between align-items-center">
                ...
                <h4 class="fw-bold mb-0 text-primary">Rp {{ (((publishForm.targetRespondents || 0) * (publishForm.honorariumPerRespondent || 0)) + 5000).toLocaleString('id-ID') }}</h4>
              </div>
            </div>
          </div>
          <div class="modal-footer border-top-0 pt-0">
            <button class="btn btn-light rounded-3" @click="showPublishModal = false" :disabled="publishing">Batal</button>
            <button class="btn btn-success rounded-3 px-4 fw-semibold" @click="proceedPublish" :disabled="publishing || !publishForm.targetRespondents">
...
.shadow-sm { box-shadow: 0 0.125rem 0.25rem rgba(0, 0, 0, 0.035) !important; }
</style>
