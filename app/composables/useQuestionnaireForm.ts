...
export const useQuestionnaireForm = () => {
   ...
   */
  const createQuestionnaire = async (projectId: string, data: CreateQuestionnaireData) => {
    loading.value = true
    error.value = null
    
    try {
      const response = await $fetch(`/api/projects/${projectId}/questionnaires`, {
        method: 'POST',
        body: {
          topic: data.topic,
   ...
   */
  const updateQuestionnaire = async (questionnaireId: string, data: UpdateQuestionnaireData) => {
    loading.value = true
    error.value = null
    ...
    try {
      ...
      if (questionnaire.value) {
        questionnaire.value = { ...questionnaire.value, ...response.questionnaire }
      }
