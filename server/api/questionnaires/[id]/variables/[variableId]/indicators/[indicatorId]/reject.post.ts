...
export default defineEventHandler(async (event) => {
    try {
        ...
        if (!await questionnaire.canEdit(user.id)) {
            throw createError({
                statusCode: 403,
                statusMessage: 'You do not have permission to reject this indicator'
            })
        }

        // Reject indicator
        await indicator.reject()

        return {
            success: true,
            message: 'Indicator rejected',
            indicator: {
                id: indicator.id,
        ...
        console.error('Reject indicator error:', error)
        throw createError({
            statusCode: 500,
            statusMessage: 'Failed to reject indicator'
        })
    }
})
